...
auth.post('/signin', async (req, res) => {
    ...
    try{
        let user = await AuthController.signin(username, password); //calling controller to get user from database
...
auth.post('/signup', async (req ,res) => {
    ...
    try{
        ...
        let user = await AuthController.signin(username, password);
        let token = AuthController.createToken(user);
        res.status(200).send({
            success: true,
            user: user,
